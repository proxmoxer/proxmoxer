...
def test_https_connection_ipv6_with_port_in_host(req_session):
    ...
    req_session.request.return_value = response
    ProxmoxAPI(
        "[fe80:1:2:3::123]:123", user="root@pam", password="secret", port=124, verify_ssl=False
    )
    call = req_session.return_value.request.call_args[1]
...
class TestSuite:
    ...
    def setup(self, session):
        ...
        self.proxmox = ProxmoxAPI(
            "proxmox", user="root@pam", password="secret", port=123, verify_ssl=False
        )
    ...
    def test_get(self):
        ...
        eq_(
            ...
            ("GET", "https://proxmox:123/api2/json/nodes/proxmox/storage/local"),
        )
